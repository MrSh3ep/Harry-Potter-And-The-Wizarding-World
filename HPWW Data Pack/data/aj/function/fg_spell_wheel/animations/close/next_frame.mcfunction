...
execute if score @s aj.close.frame matches 7.. run scoreboard players set @s aj.close.frame 1
data remove storage animated_java:temp args
execute store result storage animated_java:temp args.frame int 1 run scoreboard players get @s aj.close.frame
execute at @s run function aj:fg_spell_wheel/animations/close/zzz/apply_frame with storage animated_java:temp args
