$data modify storage pack:cooldown_api args set value {spell:"$(spell)",ticks:$(ticks)}
...
execute store result score #flash_ticks cooldown_api run data get storage pack:cooldown_api args.ticks 1
...
$execute store result storage pdb:main in.cooldowns.$(spell).selected_timer int 1 run data get storage pack:cooldown_api args.ticks 1
