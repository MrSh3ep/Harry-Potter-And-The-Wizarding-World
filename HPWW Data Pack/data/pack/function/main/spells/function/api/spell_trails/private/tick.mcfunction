...
execute as @e[type=armor_stand,tag=spell_trail] at @s if score @s spell_trail_life_ticks matches ..0 run function pack:main/spells/function/api/spell_trails/private/kill_trail

scoreboard players add #cleanup_timer spell_trail_schedule 1
execute if score #cleanup_timer spell_trail_schedule matches 20.. run function pack:main/spells/function/api/spell_trails/private/cleanup_orphan_particle_anchors
