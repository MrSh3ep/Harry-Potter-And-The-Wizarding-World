summon armor_stand ^ ^ ^-1 {Tags:[basic_cast, temp_needs_cooldown, protego_blockable], NoGravity:true, Invulnerable:true, Invisible:true, Small:true}

execute as @n[type=armor_stand, tag=temp_needs_cooldown] run scoreboard players set @s basic_cast_lifetime 16
